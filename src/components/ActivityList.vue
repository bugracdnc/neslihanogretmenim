<template>
  <div class="max-w-6xl mx-auto px-4">
    ...
    <div class="mb-10 max-w-4xl mx-auto">
      ...
      <div class="flex flex-col md:flex-row gap-4 items-center justify-between mb-4">
        ...
        <div class="relative w-full">
          ...
          <span class="absolute right-6 top-1/2 -translate-y-1/2 text-2xl opacity-60 pointer-events-none">🌸</span>
        </div>

        <div class="flex items-center bg-white/80 backdrop-blur-sm p-1.5 rounded-[1.25rem] border-2 border-pink-100 shadow-sm shrink-0">
          <button 
            @click="viewMode = 'grid'" 
    ...
    >
      ...
      >
        <div 
          class="absolute bg-gradient-to-br from-amber-50 to-pink-50 rounded-full z-0 group-hover:scale-150 transition-transform duration-700"
          :class="viewMode === 'grid' ? '-right-8 -top-8 w-32 h-32' : '-right-4 -top-4 w-16 h-16'"
        ></div>

        <div v-if="viewMode === 'grid'" class="w-full h-48 mb-5 bg-pink-50/50 rounded-2xl flex items-center justify-center shrink-0 overflow-hidden relative border border-pink-100/50">
          <img v-if="activity.thumbnail" :src="activity.thumbnail" alt="" class="w-full h-full object-cover" />
          <span v-else class="text-5xl opacity-50">🖼️</span>
        </div>
        
        <div v-else class="hidden sm:flex w-16 h-16 bg-pink-50/50 rounded-xl items-center justify-center shrink-0 overflow-hidden relative border border-pink-100/50">
          <img v-if="activity.thumbnail" :src="activity.thumbnail" alt="" class="w-full h-full object-cover" />
        ...
        <div class="relative z-10 flex-grow" :class="viewMode === 'list' && 'flex-1 min-w-0'">
          <div class="flex items-center gap-2 mb-2" :class="viewMode === 'list' && 'mb-1'">
            <span class="inline-block px-3 py-1 bg-pink-100 text-pink-700 text-[10px] sm:text-xs font-extrabold tracking-wide uppercase rounded-full whitespace-nowrap">
              {{ activity.tag }}
            </span>
          </div>
          <h3 class="font-bold text-slate-800" :class="viewMode === 'grid' ? 'text-2xl mb-2' : 'text-lg truncate'">
            {{ activity.title }}
          </h3>
          <p class="text-slate-500 leading-relaxed" :class="viewMode === 'grid' ? '' : 'text-sm line-clamp-1'">
            {{ activity.description }}
          </p>
        </div>

        <div class="relative z-10" :class="viewMode === 'grid' ? 'mt-auto pt-5 border-t border-pink-50/50 w-full' : 'w-full sm:w-auto shrink-0'">
          <button 
    ...
    <div v-if="filteredActivities.length === 0" class="text-center py-20">
      ...
      <button @click="searchQuery = ''; selectedTag = null" class="px-6 py-2 bg-pink-100 text-pink-700 rounded-full font-bold hover:bg-pink-200 transition-colors">
        Filtreleri Temizle
      </button>
    </div>
  </div>
...
// Automatically generate filter buttons based on available tags in the data
const uniqueTags = computed(() => {
  const tags = new Set(props.activities.map(a => a.tag));
  return Array.from(tags).sort();
});

// INITIALIZE: Run only on the client side (Browser) when the component mounts
onMounted(() => {
  // 1. Read View Preference from Local Storage
  const savedMode = localStorage.getItem('neslihan_view_mode');
  if (savedMode === 'list' || savedMode === 'grid') {
    viewMode.value = savedMode;
  }

  // 2. Read Search & Tag from the URL (e.g., ?tag=Matematik&search=toplama)
...
// WATCHERS: React to user changes and update the browser seamlessly

// Save view mode permanently to the device
...
watch([searchQuery, selectedTag], ([newSearch, newTag]) => {
  ...
  // Generate the new URL and use history.replaceState so we don't spam the user's back button
  const newUrl = `${window.location.pathname}${params.toString() ? '?' + params.toString() : ''}`;
  window.history.replaceState({}, '', newUrl);
});

// THE ENGINE: Filter by BOTH search text and selected tag
const filteredActivities = computed(() => {
  return props.activities.filter(a => {
    ...
    // 2. Check Tag
    const tagMatch = !selectedTag.value || a.tag === selectedTag.value;
